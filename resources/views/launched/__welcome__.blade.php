            ...
            <section class="skewed-bottom-right">
                ...
                <nav class="fixed w-full px-6 py-6 flex justify-between items-center bg-blue-800 shadow-inner z-50">
                    ...
                    <ul class="hidden absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 lg:flex lg:mx-auto lg:flex lg:items-center lg:w-auto lg:space-x-6">
                        ...
                        <li><a class="button button-primary-blue" href="{{ route('checkout.paylink') }}">Buy now - @payment('currency') @payment('price', 'default')</a></li>
                                ...
                                <div class="w-full text-center lg:text-left">
                                    ...
                                    <div class="max-w-md mx-auto lg:mx-0">
                                        <h2 class="mb-3 text-4xl lg:text-5xl text-white font-bold">
                                            <span>No more headaches to build that perfect Nova admin panel
                                        </h2>
                                    </div>
                                    <div class="max-w-sm mx-auto lg:mx-0">
                                        ...
                                        <div>
                                            <a class="inline-block mb-3 lg:mb-0 lg:mr-3 w-full lg:w-auto button button-primary-blue text-base" href="{{ route('checkout.paylink') }}">Buy now - {{ WebsiteCheckout::make()->price()->currency->symbol }} {{ WebsiteCheckout::make()->price()->amount->checkout }}</a>
                ...
                <div class="hidden navbar-menu fixed top-0 left-0 bottom-0 w-5/6 max-w-sm z-50">
                    ...
                    <nav class="relative flex flex-col py-6 px-6 h-full w-full bg-white border-r overflow-y-auto">
                        ...
                        <div class="mt-auto">
                            <div class="pt-6"><a class="block mb-2 button button-secondary-blue" href="#">Buy now - {{ WebsiteCheckout::make()->price()->currency->symbol }} {{ WebsiteCheckout::make()->price()->amount->checkout }}</a><a class="block mb-2 button button-primary-blue" href="{{ route('login') }}">Login</a></div>
...
$tweets = [
    ...
    [
        'image' => '/vendor/mastering-nova/images/david-hemphill.jpg',
    ...
    [
        'image' => '/vendor/mastering-nova/images/christoph-rumpel.jpg',
    ...
    [
        'image' => '/vendor/mastering-nova/images/caneco.jpg',
        'account' => 'caneco',
    ...
    [
        'image' => '/vendor/mastering-nova/images/patrick-brouwers.jpg',
...
@endphp
                        ...
                        @twinkle('twitter-post', ['tweet' => $tweet])
                        @endforeach
                        </div>
                        <!-- /Owl Carousel -->
                    </div>
            ...
            <section class="py-20 bg-blue-800">
                <div class="container mx-auto px-4">
                    <div class="flex flex-wrap items-center justify-center">
                        ...
                        <div class="w-full lg:w-auto lg:ml-auto text-center"><a class="inline-block leading-loose button button-primary-blue text-xl" href="{{ route('checkout.paylink') }}">Buy now - {{ WebsiteCheckout::make()->price()->currency->symbol }} {{ WebsiteCheckout::make()->price()->amount->checkout }}</a></div>
                    </div>
                </div>
            </section>
            <section id="tutorials-list">
                @foreach($series->orderBy('id', 'desc')->get() as $serie)
                    ...
                    @foreach($serie->chapters()->orderBy('index')->get() as $chapter)
                        ...
                        @if($chapter->videos->count() > 0)
                            ...
                            <div class="mb-6 w-full">
                                <div class="p-4 flex flex-wrap bg-white border-blue-800 shadow rounded-lg">
                                    <div class="w-full">
                                        <h3 class="font-heading mb-3 font-bold">{{ $video->title }}</h3>
                                        <div class="flex items-center gap-2">
                                            @if($video->is_active)
                                            <p class="text-white text-xs rounded-full px-2 py-1 bg-blue-800">Recorded</p>
                                            @else
                                            <p class="text-white text-xs rounded-full px-2 py-1 bg-gray-500">Being recorded</p>
                                            @endif
                                            @if($video->is_free)
                                            <a class="text-xs rounded-full px-2 py-1 bg-blue-200 text-blue-800" href="#">Watch for free</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
            ...
            <section id="faq">
                <div class="py-20 bg-blue-800 radius-for-skewed">
                    <div class="container mx-auto px-4">
                        <div class="mb-16 max-w-xl mx-auto text-center">
                            <h2 class="text-white text-5xl font-bold">Any doubts?</h2>
                        </div>
                        <div class="flex flex-wrap -mx-4 -mb-8">
                        @php
$questions = [
    ['Do I need to have experience on Nova before buying this course?', 'Not at all. I cover the basics and then move into advanced areas further. I recommend you watch the videos in the recommended order.'],
     ['Can I pay with Paypal?', 'Sure! I use Paddle and they support tons of payment methods including Paypal.'],
     ['Is it VAT included?', 'No, it\'s VAT excluded. As soon as you select your country the VAT will be applied.'],
     ['Can I see some of your videos before buying?', 'Yes, I do post free videos per Chapter, so you can have an idea about the content quality. Feel free to click on the "watch videos" and see the free ones to get an idea about what is waiting for you :)'],
     ['Can I order a refund?', 'Absolutely. No questions asked. I only want you to pay if you see it\'s valuable for you.'],
     ['Do you use purchase power parity?', 'In case you want a discount based on your country, just run the url with /ppp at the end.'],
];
                        @endphp
                        @foreach($questions as $question)
                            <div class="w-full lg:w-1/2 px-4 mb-8">
                                <div class="h-full p-8 bg-white rounded">
                                    <div class="mb-6 flex items-start">
                                        <span class="mr-4 inline-block p-3 rounded-full bg-blue-600">
                                            <svg class="w-6 h-6 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewbox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z"></path>
                                            </svg>
                                        </span>
                                        <h3 class="text-xl font-bold">{{ $question[0] }}</h3>
                                    </div>
                                    <p class="text-gray-400 leading-loose">{{ $question[1] }}</p>
                                </div>
                            </div>
                        @endforeach
                        </div>
            ...
            <section>
                <div class="py-10 bg-gray-50 radius-for-skewed">
                    <div class="max-w-md mx-auto text-center">
                        <p class="mb-6 text-sm font-semibold text-gray-400">{{ \Illuminate\Foundation\Inspiring::quote() }}</p>
                        <div class="flex space-x-2 lg:space-x-4 justify-center">
                            <a href="https://twitter.com/brunocfalcao" target="_blank">
                                <x-feathericon-twitter class="text-blue-500"/>
                            </a>
                        </div>
                    </div>
